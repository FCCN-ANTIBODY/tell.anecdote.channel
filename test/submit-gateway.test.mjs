...
// (c) allowlist exactly the one repo's issues/comments paths, (d) inject the credential
...
// (c) path allowlist — exactly this repo's issues surface
const GOOD = "/repos/FCCN-ANTIBODY/tell.anecdote.channel/issues";
for (const bad of [
  "/repos/FCCN-ANTIBODY/other-repo/issues",
...
// (d) the relay itself — issue mode and comment mode
calls.length = 0;
const block = { title: "poll: budget", body: "```tell\n{...}\n```" };
...
r = await worker.fetch(req("POST", "/submit", { path: GOOD + "/9/comments", body: { body: "c" } }), ENV);
assert(r.status === 201, "comment-mode relay refused");
...
console.log("ok: submit-gateway relays verbatim, allowlists one repo, never leaks the credential");
