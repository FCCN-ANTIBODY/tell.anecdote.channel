...
- **First secret-bearing worker in the constellation.** The feed/piles gateways are stateless
  projections; this one holds a credential. Its blast radius equals the PAT's: create issues and
  comments on one already-public repo. The path allowlist in `worker.js` is defense in depth on
  top of GitHub's own scope boundary.
...
```sh
bin/qr --pile cd04-q1 --poll budget --submit-url https://tell.anecdote.channel/submit
...
curl -s -X POST https://tell.anecdote.channel/submit \
  ...
  -d '{"path":"/repos/FCCN-ANTIBODY/tell.anecdote.channel/issues","body":{"title":"relay smoke","body":"..."}}'
# → 201 {"html_url": "...", "number": N}   (403 path-not-allowed for anything off-repo)
```
